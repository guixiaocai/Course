...
\noindent \textbf{2. Give an example of heuristic, which is admissible but not consistent.}

\textbf{Solution} 给定一个admissible的heuristic的例子为：$h(n)=\llfloor h^*(n)\rrfloor$.

\indent 对于下图来说：

\qquad \includegraphics[width=2.5in]{11_1.PNG}
...
\noindent \textbf{3.Judge whether the following result is true. Prove your argument.}

	$$A* of graph search is optimal with admissible heuristic$$

\indent \textbf{Solution} 以上结论是错误的, 证明如下.

\textbf{Proof} 以\textbf{题2}中的heuristic：$h(n)=\llfloor h^*(n)\rrfloor$ 为例，来证明此结论错误.

\indent 以下图为例：\\
\indent \indent \includegraphics[width=3in]{11_2.PNG}
